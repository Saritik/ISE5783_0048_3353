...
public class Triangle extends Polygon {
	...
	public List<GeoPoint> findGeoIntersectionsHelper(Ray ray, double maxDistance) {
		...
		if (alignZero(n1.dotProduct(ray.getDir())) > 0 && alignZero(n2.dotProduct(ray.getDir())) > 0
				&& alignZero(n3.dotProduct(ray.getDir())) > 0) {
			rayPoints.get(0).geometry = this;
			return rayPoints;
		}
		// The point is outside if all 𝒗 ∙ 𝑵𝒊 have the same sign (+/-)
		else if (alignZero(n1.dotProduct(ray.getDir())) < 0 && alignZero(n2.dotProduct(ray.getDir())) < 0
				&& alignZero(n3.dotProduct(ray.getDir())) < 0) {
			rayPoints.get(0).geometry = this;
			return rayPoints;
		}
		if (isZero(n1.dotProduct(ray.getDir())) || isZero(n2.dotProduct(ray.getDir()))
